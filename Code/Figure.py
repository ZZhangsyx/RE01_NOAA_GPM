...
def get_score(processed_path, DEA_path, Basin, Scale, Basin_name, BN, BS):
    ...
    GPM_F = pd.read_csv(os.path.join(processed_path, 'GPF_' + BN + '.csv'))

    csv['Lat'], csv['Lon'], csv['GPM_' + BS] = Lat, Lon, GPM
    ...
    mean_pre = np.zeros([len(lat_bs), len(lon_bs), 6, 6])
        ...
        for jlon in np.arange(len(lon_bs)):
            if isBasin[ilat, jlon]:
                ...
                if csv_select.shape[0] != 0:
                    ...
                    csv_select['Time'] = pd.to_datetime(csv_select['Time'])
                    csv_select = csv_select.set_index('Time')

                    Loss_F = np.array((GPM_F['Lat'] - lat_grid) ** 2 + (GPM_F['Lon'] - lon_grid) ** 2)
                    select_index_F = np.where(Loss_F == np.min(Loss_F))[0]
                    GPM_select_F = GPM_F.iloc[select_index_F]
                    GPM_select_F['Time'] = pd.to_datetime(GPM_select_F['Time'])
                    GPM_select_F = GPM_select_F.set_index('Time')
                    Pred_cat = pd.concat(
                        [csv_select, GPM_select_F], axis=1,
                        join='inner')

                    mean_pre[ilat, jlon, :, 5] = Display_Score(Pred_cat['GPM-F'], csv_select['CMPA_' + BS])
...
def Boxplot(in_path, out_path, BN):
    ...
    """
    DATA = np.load(in_path + '/Score.npy')
    ...
    for i in range(2):
        for j in range(3):
            ...
            data = []
            for k in range(6):
                data_t = np.reshape(DATA[:, :, index, k], -1)
                data_t = list(filter(lambda x: x != 0, data_t))
                data.append(data_t)
...
def ImportancePlot(model_path, out_path):
    ...
    title_name = ['a) Classifier in DJR', 'b) Regressor in DJR', 'c) Classifier in FHR', 'd) Regressor in FHR']
    feature_name = [
...
def ScorePlot(in_path, out_path, Basin_DJ, Basin_FH, Scale, Figname):
    # load the score data
    score_DJ = np.load(os.path.join(in_path, 'processed_DJ2', 'Score.npy'))
    ...
    cmap = ListedColormap(['white', 'aquamarine', 'cyan', 'lime', 'limegreen',
                           'yellow', 'gold', 'darkorange', 'r', 'firebrick','darkred'])
    ...
    if Figname == 'RMSE':
        ...
        for j in range(5):
            ...
            axes[j].text(110.1, 39.1, text[j] + ') ' + labels[j], ha='left', fontsize=fontsize,
                         wrap=True)
        ...
        for i in range(2):
            ...
            for j in range(5):
                ...
                if index < 5:
                    ...
                    axes[i, j].text(113.6, 25.1, text0[j] +')', ha='left', fontsize=22, wrap=True)
                else:
                    axes[i, j].text(113.6, 25.1, text1[j] +')', ha='left', fontsize=22, wrap=True)
        ...
        for i in range(2):
            ...
            for j in range(5):
                ...
                if index < 5:
                    ...
                    axes[i, j].text(113.6, 25.1, text0[j]+')', ha='left', fontsize=20, wrap=True)
                else:
                    axes[i, j].text(113.6, 25.1, text1[j]+')', ha='left', fontsize=20, wrap=True)
    ...
    if Figname == 2:
        ...
        for i in range(2):
            ...
            if i == 1:
                data = data + 0
            levels = MaxNLocator(nbins=11).tick_values(0.4, 0.9)
            norm = BoundaryNorm(levels, ncolors=cmap.N, clip=True)
            for j in range(5):
                index = 5 * i + j
    ...
    if Figname == 3:
        ...
        for i in range(2):
            ...
            for j in range(5):
                ...
                axes[i, j].text(110.1, 39.1, text[index] + ')', ha='left', fontsize=20, wrap=True)
...
def TimeseriessScatter(in_path, processed_path, sta_path, out_path, BN, Si):
    csv_name = os.path.join(in_path, 'STA_' + BN + '.csv')
    data_pro = pd.read_csv(csv_name)
    data_pro['Time'] = pd.to_datetime(data_pro['Time'])
    data_pro = data_pro.set_index('Time')
    ...
    for i in range(2):
        for j in range(3):
            ...
            axes[i, j].scatter(Y, X, c=colors[index])
            axes[i, j].text(5, 45, t, ha='left', fontsize=16, wrap=True)
            axes[i, j].xaxis.set_tick_params(labelsize=16)
            axes[i, j].yaxis.set_tick_params(labelsize=16)
            plt.ylim([0, 100])
            ...
            if index >= 3:
                axes[i, j].xaxis.set_ticks(np.arange(0, 101, 20))
                axes[i, j].xaxis.set_ticklabels(['0', '20', '40', '60', '80', '100'])
    fig.subplots_adjust(hspace=0.1, wspace=0.1)
    plt.savefig(out_path + '/TimeseriesScatter.png')
    plt.show()


def Statisticplot():
    XGB_DJR = [0.769, 0.794, 0.384, 0.694, 6.023 / 20, 0.606]
    GPM_DJR = [0.675, 0.858, 0.495, 0.636, 13.373 / 20, 0.575]
    XGB_FHR = np.negative([0.871, 0.716, 0.369, 0.671, 3.581 / 20, 0.600])
    GPM_FHR = np.negative([0.549, 0.884, 0.726, 0.418, 10.565 / 20, 0.437])
    ...
    rects1 = plt.bar(index, GPM_DJR, bar_width,
                     alpha=opacity,
                     color='firebrick',
                     error_kw=error_config,
                     label='GPM-E DJR')

    rects2 = plt.bar(index + bar_width, XGB_DJR, bar_width,
                     alpha=opacity,
                     color='goldenrod',
                     error_kw=error_config,
                     label='XGB DJR')
    rects3 = plt.bar(index, GPM_FHR, bar_width,
                     alpha=opacity / 2,
                     color='firebrick',
                     error_kw=error_config,
                     label='GPM-E FHR')

    rects4 = plt.bar(index + bar_width, XGB_FHR, bar_width,
                     alpha=opacity / 2,
                     color='goldenrod',
                     error_kw=error_config,
                     label='XGB FHR')

    XGB_DJR = [0.769, 0.794, 0.384, 0.694, 6.023, 0.606]
    GPM_DJR = [0.675, 0.858, 0.495, 0.636, 13.373, 0.575]
    XGB_FHR = [0.871, 0.716, 0.369, 0.671, 3.581, 0.600]
    GPM_FHR = [0.549, 0.884, 0.726, 0.418, 10.565, 0.437]
    ind = 0
    for rects, data in zip([rects1, rects2, rects3, rects4], [GPM_DJR, XGB_DJR, GPM_FHR, XGB_FHR]):
    ...
    plt.yticks(np.linspace(-1, 1, 11),
               ...
               fontsize=axissize)
    plt.xticks(index + bar_width / 2, ('ACC', 'POD', 'FAR', 'F', 'RMSE(mm/day)', 'R'), fontsize=axissize)
...
def Scatter_plot(in_path, out_path, BN, BS):
    data = pd.read_csv(in_path + '/Test_reg' + BN + '.csv')
    data['Time'] = pd.to_datetime(data['Time'])
    data = data.set_index('Time')
    data = data['2018']

    GPM = pd.read_csv(in_path + '/DEA/GPM.csv')
    GPM['Time'] = pd.to_datetime(GPM['Time'])
    GPM = GPM.set_index('Time')
    GPM = GPM['2018']

    text = ["f) SVM", "g) RF", "h) ANN", "i) XGB", "j) GPM"]
    colors = ['cyan', 'lime', 'limegreen', 'yellow', 'darkorange', 'r']
    CMPA, SVM, RF, ANN, XGB = data['CMPA_' + BS], data['SVM_cat'], data['RF_cat'], data['ANN_cat'], data['XGB_cat']
    fig, axes = plt.subplots(nrows=1, ncols=5, figsize=(6, 6), sharey=True, sharex=True)
    plt.xlim([0, 200])
    plt.ylim([0, 200])
    plt.xticks(np.arange(0, 200, 50), ['0', '50', '100', '150'])
    axes[4].scatter(CMPA, GPM, alpha=.8, c=colors[4], label='CMPA')
    axes[0].scatter(CMPA, SVM, alpha=.8, c=colors[0], label='SVM')
    axes[1].scatter(CMPA, RF, alpha=.8, c=colors[1], label='RF')
    axes[2].scatter(CMPA, ANN, alpha=.8, c=colors[2], label='ANN')
    axes[3].scatter(CMPA, XGB, alpha=.8, c=colors[3], label='XGB')
    for i in range(5):
        axes[i].xaxis.set_tick_params(labelsize=18)
        axes[i].yaxis.set_tick_params(labelsize=18)
        axes[i].set_aspect('equal', adjustable='box')
        axes[i].text(10, 180, text[i], ha='left', fontsize=22, wrap=True)
    fig.subplots_adjust(bottom=0.2, right=0.96, top=0.8, left=0.06, hspace=0.2, wspace=0.02)
    plt.savefig(out_path + "/Scatterplot_" + BN)
    plt.show()


def main():
    Basin_name = 'DongJiang'
    BN = 'FH'
    BS = 'F'
    in_path = '../data'
    ...
    # get_score(processed_path, DEA_path, Basin_DJ, SpaceScale, Basin_name, BN, BS)
    # Boxplot(processed_path, figure_path, BN)
    # Scatter_plot(processed_path, figure_path, BN, BS)
    # ImportancePlot(model_path, figure_path)
    # for i in Figindex:
    ScorePlot(in_path, figure_path, Basin_DJ, Basin_FH, SpaceScale, 5)
    # DistributionPlot(figure_path)
    # TimeseriesPlot(DEA_path, processed_path, sta_path, figure_path, BN, Si)
    # TimeseriessScatter(DEA_path, processed_path, sta_path, figure_path, BN, Si)
    # Statisticplot()
    # Statisticplot_reg()
