...
def Display_Score(data, label, isShow=True):
    ...
    if isShow:
        ...
        print("F：   ", '%.3f' % F, '\n')
    return MSE, RMSE, CC, POD, FAR, accuracy, F
...
def Train_regression(in_path, out_path, BN, BS, isBalance):
    ...
    CMPA = pd.read_csv(os.path.join(in_path, 'CMPA.csv'))
    ...
    if not isBalance:
        ...
        Test2 = pd.read_csv(os.path.join(out_path, 'Test_class' + BN + '.csv'))
        ...
        data = pd.read_csv(os.path.join(in_path, 'GPM.csv'))
        ...
        Test2.to_csv(os.path.join(out_path, 'Test_reg' + BN + '.csv'), index=True)
        df_results.to_csv(os.path.join(out_path, 'Results_reg' + BN + '.csv'), index=True)
    else:
        print('Execute modeling..')
        Train2 = pd.read_csv(os.path.join(out_path, 'Train_class' + BN + '.csv'))
        Test2 = pd.read_csv(os.path.join(out_path, 'Test_class' + BN + '.csv'))
        df_results = pd.DataFrame()
        ...
        Train2.to_csv(os.path.join(out_path, 'Train_reg' + BN + '.csv'), index=True)
        Test2.to_csv(os.path.join(out_path, 'Test_reg' + BN + '.csv'), index=True)
        df_results.to_csv(os.path.join(out_path, 'Results_reg' + BN + '.csv'), index=True)
    print('Code deal!')
