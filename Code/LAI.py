...
in_path = r'I:\RE01_NOAA_GPM\data\Lai.npy'
out_path = '../figure'
Figname = 'LAI(%)'
Basin_FH = [35.3, 39.0, 110.5, 113.6]
Basin_DJ = [22.5, 25.5, 113.5, 116]
Scale = 0.1

# load the score data
data = np.load(in_path)[::-1,:]
data[data <0] = np.nan

lat_bs = np.linspace(Basin_FH[0], Basin_FH[1], data.shape[0])
lon_bs = np.linspace(Basin_FH[2], Basin_FH[3], data.shape[1])
