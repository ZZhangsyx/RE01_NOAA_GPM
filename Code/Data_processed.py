...
def Csv_DEA(in_path, out_path, Basin_name, BN):
    ...
    data_nan.dropna(axis='columns', thresh=data.shape[0]*drop_thresh, inplace=True)
    data_nan['isRain_CMPA'] = isRain_CMPA
    data_nan['CMPA_' + Basin_name] = CMPA
    data_nan['GPM_' + Basin_name] = GPM
    data_nan.dropna(axis=0, thresh=data.shape[1]*drop_thresh, inplace=True)
...
def main():
    ...
    save_path = '../data'
    Raw_path = r'I:\RE01_NOAA_GPM\data\Raw'
